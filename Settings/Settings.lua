...
function Settings:BuildTree()
	...
	for sName,mod in pairs(self.core.modules) do
		if not self.instances[mod.instance] then 
			self.instances[mod.instance] = {}
		end 

		self.instances[mod.instance][mod.displayName] = sName
	end 

	for strInstance,tBosses in pairs(self.instances) do 
		...
		for strEncounter,strModule in pairs(tBosses) do
			...
			self:StyleButton(wndEncounterGroupBtn,"encounter",self.core.modules[strModule].config.enable)
		...
		local bInstanceGroupHasChildren = #wndInstanceContents:GetChildren() > 0
	...
	for mainKey,wndMainGroup in pairs(wndLeftScroll:GetChildren()) do
		...
		if wndMainButton:IsChecked() then
			for key, wndTopGroup in pairs(wndMainContents:GetChildren()) do
				local wndTopButton = wndTopGroup:FindChild("EncounterBtn")
...
function Settings:BuildSoundDropdown(wnd,setting,value)
	...
	for idx,fileName in ipairs(self.soundFiles) do
		...
		local item = Apollo.LoadForm(self.xmlDoc, item, wnd:FindChild("ChoiceContainer"), self)
		item:SetText(fileName)
		item:SetCheck(fileName == value)
		item:SetAnchorOffsets(42,49 + offset,-44,81 + offset)
		item:AddEventHandler("ButtonCheck", "OnSoundChoose", self)
	end
