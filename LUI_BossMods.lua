...
function LUI_BossMods:CheckZone(tZone)
	if not tZone or not self.zone then 
		return 
	end 
	...
	if self.zone.continentId == tZone.continentId then
		...
		if type(tZone.mapId) == "number" then
			if tZone.mapId == 0 then
				return true
			else
				if tZone.mapId == self.zone.mapId then
					return true
				end
			end
		elseif type(tZone.mapId) == "table" then
			for _,id in pairs(tZone.mapId) do
				if id == self.zone.mapId then
					return true
				end
			end
		end
...
function LUI_BossMods:SearchForEncounter()
	...
	for sName,tModule in pairs(self.modules) do
		local state = self:CheckZone(tModule.zone)
...
function LUI_BossMods:OnUnitCreated(unit)
	if self.bIsRunning == true  then 
		...
		if unit:IsThePlayer() then
			...
			if not self.tSavedUnits[unit:GetId()] then 
				self.tSavedUnits[unit:GetId()] = unit
			end 
		end
	end 
end 
...
function LUI_BossMods:OnUnitDestroyed(unit)
	if self.bIsRunning == true then 
		...
		if self.tSavedUnits ~= nil and self.tSavedUnits[unit:GetId()] ~= nil then 
			self.tSavedUnits[unit:GetId()] = nil
		end 
	end 
end 
...
function LUI_BossMods:ProcessSavedUnits()
	...
	for _,unit in pairs(self.tSavedUnits) do 
		self:OnUnitCreated(unit)
	end
end
...
function LUI_BossMods:UpdateTimer(tTimer)
	...
	local nProgress = nRemaining / 100
...
function LUI_BossMods:ShowCast(tCast,sName,sColor)
	...
	if not sName then 
		sName = (tCast.sName == "MOO") and (tCast.sUnitName or "") or (tCast.sName or "")
	end 

	self.wndCastbar:FindChild("Name"):SetTextColor(self.config.castbar.textColor)
	self.wndCastbar:FindChild("Name"):SetText(sName)
...
function LUI_BossMods:DrawLineBetween(Key, FromOrigin, ToOrigin, nWidth, sColor, nDuration, nNumberOfDot, fHandler, tData)
	if not self.wndOverlay then 
		self:LoadWindows()
	end
...
function LUI_BossMods:GetDistance(FromOrigin, ToOrigin)
